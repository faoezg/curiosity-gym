...
# 
#                     # TODO INVESTIAGE BUG
#                     #   File "H:\Master_thesis\code\curiosity-gym\src\curiosity_gym\core\pov\local_view.py", line 60, in transform_obs
#                     #     local[ix_new] = state[ix]
#                     #                     ~~~~~^^^^
#                     # IndexError: index 161 is out of bounds for axis 0 with size 161
#                     # ALSO BUG WITH VIEWPORT WHEN WALLS ARE IN VIEW
#                     if ix < 0 or ix >= len(state) or (not self.is_visible(state, pos, cell) and not self.xray):
#                         continue
# 
#                     self.visible_positions.append(cell)
#                     local[ix_new] = state[ix]
